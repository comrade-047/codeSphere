...
import ProblemDiscussionTab from "../Discussions/ProblemDiscussionTab";
...
export default function ProblemTabs({ problem, submissions, isLoggedIn, setSelectedSubmission }) {
  ...
  const tabs = [
    { key: "problem", label: "Problem", icon: <BookOpen className="w-4 h-4 mr-1.5" /> },
    { key: "submissions", label: "Submissions", icon: <History className="w-4 h-4 mr-1.5" /> },
    { key: "discussion", label: "Discussion", icon: <MessageSquare className="w-4 h-4 mr-1.5" /> },
  ];

  return (
    <div className="flex flex-col h-screen bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-700 rounded-lg overflow-hidden">
      {/* Tabs Header (Sticky) */}
      ...
      <div className="flex-1 overflow-y-auto px-2 py-2">
        ...
        {activeTab === "discussion" && (
          <ProblemDiscussionTab isLoggedIn={isLoggedIn} problemId={problem._id} />
        )}
      </div>
    </div>
  );
